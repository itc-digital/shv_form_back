...
class FormAction {
    public function validate($f3, $data)
    {
        ...
        elseif (mb_strlen($data['groupa'])<0){return array ('status' => False, 'textError' => "Поле группа пустое");}
        ...
        elseif (mb_strlen($data['groupa'])>1024){return array ('status' => False, 'textError' => "Поле группа переполнено");}
    ...
    public static function record($f3, $data) {
        $f3->get('DB')->exec('INSERT INTO `records`(`fio`, `fakultet`, `kurs`, `groupa`, `phone`, `vkLink`, `experience`, 
